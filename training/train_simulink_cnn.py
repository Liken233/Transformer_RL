...
"""
================================================================================
CNNPPO 训练脚本 - 基于 1D-CNN 策略网络的强化学习训练
================================================================================
与 Transformer / LSTM 版本完全对标，仅将策略网络替换为 1D-CNN。

使用方式：
    # 单卡训练（自动选择最佳 GPU）
    python train_simulink_cnn.py --target_type random \
        --stack_frames 8 --feature_dim 5 --num_envs 16 --device auto

    # 指定 GPU
    CUDA_VISIBLE_DEVICES=0 python train_simulink_cnn.py ...

    # 三种网络对比实验
    CUDA_VISIBLE_DEVICES=0 python train_simulink_transformer.py --exp_name tf_exp0 ... &
    CUDA_VISIBLE_DEVICES=1 python train_simulink_lstm.py --exp_name lstm_exp0 ... &
    CUDA_VISIBLE_DEVICES=2 python train_simulink_cnn.py --exp_name cnn_exp0 ... &
"""
...
# 导入 CNN 策略
try:
    from policy.CNNPolicy import CNNActorCriticPolicy
    CNN_AVAILABLE = True
except ImportError:
    print("[ERROR] 无法导入 CNNPolicy")
    raise


# =====================================================================
# 0. 硬件检测与配置（与 Transformer / LSTM 版本完全一致）
# =====================================================================
...
# =====================================================================
# 1. 回调（与 Transformer / LSTM 版本完全一致）
# =====================================================================
...
# =====================================================================
# 2. 环境创建（与 Transformer / LSTM 版本完全一致）
# =====================================================================
...
# =====================================================================
# 3. 评估函数（与 Transformer / LSTM 版本完全一致）
# =====================================================================
...
def main():
    parser = argparse.ArgumentParser(description='CNNPPO 训练脚本')
    ...
    # CNN 配置（与 Transformer / LSTM 参数对标）
    ...
    # 解析 CNN 特有参数
    args.channels = [int(x.strip()) for x in args.channels.split(',')]
    args.kernel_sizes = [int(x.strip()) for x in args.kernel_sizes.split(',')]
    ...
    if args.exp_name is None:
        ...
        exp_name = f"{time_str}_{args.target_type}_cnn_nenv{args.num_envs}_len{args.stack_frames}"
    else:
    ...
    print(f"\n[策略] CNNActorCriticPolicy")
    print(f"[策略] seq_len={args.stack_frames}, feature_dim={args.feature_dim}")
    print(f"[策略] latent_dim_pi={args.latent_dim_pi}, channels={args.channels}, kernel_sizes={args.kernel_sizes}")
    print(f"[策略] use_bn={args.use_bn}")
