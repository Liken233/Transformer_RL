...
"""
================================================================================
LSTMPPO 训练脚本 - 基于 LSTM 策略网络的强化学习训练
================================================================================
与 Transformer 版本完全对标，仅将策略网络从 Transformer 替换为 LSTM。

使用方式：
    # 单卡训练（自动选择最佳 GPU）
    python train_simulink_lstm.py --target_type random \
        --stack_frames 8 --feature_dim 5 --num_envs 16 --device auto

    # 指定 GPU
    CUDA_VISIBLE_DEVICES=0 python train_simulink_lstm.py ...

    # 与 Transformer 对比实验
    CUDA_VISIBLE_DEVICES=0 python train_simulink_lstm.py --exp_name lstm_exp0 ... &
    CUDA_VISIBLE_DEVICES=1 python train_simulink_transformer.py --exp_name tf_exp0 ... &
"""
...
# 导入 LSTM 策略
try:
    from policy.LSTMPolicy import LSTMActorCriticPolicy
    LSTM_AVAILABLE = True
except ImportError:
    print("[ERROR] 无法导入 LSTMPolicy")
    raise


# =====================================================================
# 0. 硬件检测与配置（与 Transformer 版本完全一致）
# =====================================================================
...
def main():
    parser = argparse.ArgumentParser(description='LSTMPPO 训练脚本')
    ...
    if args.exp_name is None:
        ...
        exp_name = f"{time_str}_{args.target_type}_lstm_nenv{args.num_envs}_len{args.stack_frames}"
    else:
    ...
    print(f"\n[策略] LSTMActorCriticPolicy")
    print(f"[策略] seq_len={args.stack_frames}, feature_dim={args.feature_dim}")
    print(f"[策略] latent_dim_pi={args.latent_dim_pi}, hidden_size={args.hidden_size}, num_layers={args.num_layers}")
    print(f"[策略] bidirectional={args.bidirectional}")
